...
def initialize_sample_data():
    """
    Automatically populate the data_store directory with a few sample assets.
    This ensures the app has at least some data available for visualization.

    Downloads data for a small sample universe (SPY, QQQ, TLT) if missing.
    """
    sample_assets = ["SPY", "QQQ", "TLT"]
    data_path = get_data_path()

    print("\nInitializing sample data in data_store...\n")
        ...
        if not file_path.exists():
            try:
                ...
                if not df.empty:
                    ...
                    print(f"✅ Saved {ticker}.parquet to {data_path}")
                else:
                    print(f"⚠️ No data returned for {ticker}")
            except Exception as e:
                print(f"❌ Error downloading {ticker}: {e}")
        else:
            print(f"✅ {ticker}.parquet already exists, skipping.")
...
if __name__ == '__main__':
   if __name__ == '__main__':
    ...
    # Example usage
    ohlcv = fetch_ohlcv_data(force_download=False)
    print("\nOHLCV Data Head (SPY):")
    if 'SPY' in ohlcv:
        print(ohlcv['SPY'].head())

    fred_data = fetch_fred_data(force_download=False)
    if fred_data:
